...
\setbeamertemplate{footline}{%
  \begin{beamercolorbox}[wd=\paperwidth,ht=2.4cm]{footline}%
    \begin{tikzpicture}
      \fill[CornellDark] (0,0) rectangle (\paperwidth, 2cm);
      % References row
      \node[text=white!75, font=\fontsize{12}{14}\selectfont,
...
\begin{frame}[t]
	...
	\begin{columns}[T,totalwidth=\textwidth]
		...
		\begin{column}{0.31\textwidth}
			...
			\begin{tcolorbox}[posterblock, title={Introduction}]
				...
				\vspace{0.15cm}
				\small\hl{DoRA} (Liu et al., 2024) decouples them: it learns weight magnitude separately while using LoRA to update the direction.
				\par\vspace{0.15cm}
				\small\textbf{\textcolor{black}{Goal:}} Recreate DoRA NLP paper results + extend to non-NLP modalities.
				\par\vspace{0.15cm}
				\small\textbf{\textcolor{black}{Methodology:}} 4 models, 6 benchmarks for LoRA/DoRA direct comparison.
			\end{tcolorbox}

			\begin{tcolorbox}[posterblock, title={DoRA Architecture}]
				\begin{center}
					\includegraphics[width=0.65\linewidth]{dora_architecture.png}
				\end{center}
				\vspace{0.15cm}
				\small\textit{DoRA splits each weight $W_0$ into magnitude $m$ and direction $V/\|V\|$, applying a LoRA update $BA$ to the direction while training magnitude separately.}
			\end{tcolorbox}
			...
			\begin{tcolorbox}[posterblock,title=Audio Results]
				...
				\end{center}
				{\small\textit{\hl{-44.6\% avg tr-loss.} DoRA  faster optimization and smoother convergence.}}
			\end{tcolorbox}
		...
		\begin{column}{0.37\textwidth}
			...
			\begin{tcolorbox}[posterblock,title=NLP Results]
				...
				\small\textit{DoRA's advantage is largest on \hl{low-data tasks}
					...
					gradient interference.  Matches original paper results.}
				...
				{\footnotesize
					\begin{tabularx}{\linewidth}{@{} l X r @{}}
						...
						\textbf{MRPC}
						 ...
						 & $\to$ \hl{Equiv.}                                                                                       \\[2pt]
						\textbf{RTE}
						 & P: ``\textit{Leaders signed the Egypt–Israel treaty in 1979.}''\ \ H: ``\textit{War started in 1979.}''
						 & $\to$ \hl{Not Entailed}                                                                                 \\
					\end{tabularx}}
				...
				\vspace{0.15cm}
				\small\textit{\hl{corr: Full = 0.40, LoRA = 0.93, DoRA = 0.24}. DoRA shows significantly lower slope and correlation than LoRA, closer to FFT, showing the effect of de-coupling $\bold{M}$ and $\bold{D}$.}
		...
		\begin{column}{0.29\textwidth}
			...
			\begin{tcolorbox}[posterblock,title=Vision \& Robot Results]
				...
				{\small Green = ground truth\quad Red = predicted grasp}
			...
			\begin{tcolorbox}[posterblock,title=Conclusions]
				{\small\begin{itemize}\setlength{\itemsep}{0.04cm}\setlength{\topsep}{0pt}
						...
						\item \hl{Audio:} $+0.7\%$ test acc.\ and 10\% faster training on
